...
var moduleEventsHome = ( function () {
    ...
    async function addPhotoPostsToLocalStorage() {
        var photoPosts = await moduleScript.loadPhotoPosts();
        var users = await moduleScript.loadUsers();
    ...
    async function addButton(editPost) {
        await moduleScript.addPhotoPost(editPost);
    }
        ...
        document.getElementById('Add').addEventListener('click', (event) => {
            ...
            document.getElementById('save').addEventListener('click', (event) => {
                ...
                /*  fetch('addPost', {
                  ...
                  id = (parseInt(moduleScript.searchMaxId(JSON.parse(localStorage.getItem('posts')))) + 1).toString();
                var newPost =   {
                    author: username,
                ...
                var editPosts = myModule.addPhotoPost(JSON.parse(localStorage.getItem('posts')), username,newPost);
    ...
    async function deletePhotoPost(post) {
       await  moduleScript.removePhotoPost(Number(post.getAttribute('data-id')));
    }

    async function editPhotoPost(post) {
        await  moduleScript.editPhotoPost(post);
    }

    function editPostFunct() {
        document.querySelector('.site_content').addEventListener('click', (event) => {
            ...
            if (event.target.className === 'red') {
                ...
                document.getElementById('save').addEventListener('click', (event) => {
                    ...
                    var editPosts = myModule.editPhotoPost(postHtml.getAttribute('data-id'), JSON.parse(localStorage.getItem('posts')), username,newItem );
                    editPhotoPost(newItem);
                    localStorage.setItem('posts', JSON.stringify(editPosts));
                    event.preventDefault();
                });
                event.preventDefault();
            ...
            if (event.target.className === 'delete') {
               ...
               //  moduleScript.removePhotoPost(Number(post.getAttribute('data-id')));

                    myModule.changeTitletoMyPage();
                    deletePhotoPost(post);
                    myModule.getPhotoPosts(JSON.parse(localStorage.getItem('posts')), username, 0, 10, { author: username });
                    setPagination();
                    event.preventDefault();
                });
...
moduleEventsHome.editPostFunct();
moduleEventsHome.downloadButton();
moduleEventsHome.userTop();
moduleEventsHome.registrationFunction();
moduleEventsHome.showLenta();
moduleEventsHome.showMyPage();
moduleEventsHome.searchButton();
moduleEventsHome.userNameFunc();
moduleEventsHome.addPostToPage();